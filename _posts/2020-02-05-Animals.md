...
I'm going to drop the anima's names, and the 'OutcomeSubtype' is not necessary because it does not provide any additional information: it's only known after the 'Outcometype' has been determined (it is only a qualitative precision after the animal departs and including it might create a bias in the characteristics). 

I'm going to drop the animal ID for the moment. 

First of all, I'm make the SexUponOutcome 'atomic' (since it contains two information at the same time: whether the animal has been castrated and whether itis male or female). I'll split it into two columns. 

AgeUponOutcome represents the age in time units (years, months, weeks, days) we will therefore adopt a year-base convention: $1 \text{ year} \approx 365 \text{ days} \approx 52 \text{ weeks}$.

All the variables we have at hand (except for the age) are categorical variables. Thus we will convert them to numerical values later on.
