...
I'd also like to apologize in advance for some deliberate technical omissions in the dataset above (and the code snippets) though I will try to summarize the methodology as thoroughly as I can.
...
Another common nuisance:

4. Common words

While I'm not going to detail the pre-processing in this article, I'd just like to 

We need to address the fact that the ‘Content’ column comes in both languages Arabic and English (and sometimes a mix of the two).  A possible way to do this is to use one of the Translation Services (however, these are paid services (to be discussed)). Another way to do this is to research available Arabic Sentiment Lexicons (and therefore use the Stopwords package lexicon for example).
The first step would be to unnest the words available in the ‘Content’ column and filter them using the DetectLanguage function. We can later consider grouping by Medium before removing stop words and using an English Lexicon (Afinn, bing, Loughran).
We can also group the data by ‘Inbound’ and ‘Outbound’ status in order to compare customer sentiment and employee feedback. 
As a first step, I have worked with the ‘Content’ in English only.
