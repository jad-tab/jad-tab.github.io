...
Of course, these analyses are far from being thorough and exhaustive: we made some choices that severely restricted the data (by not considering the posts in Arabic). We then based all the analysis on a single lexicon (which is itself limited and subjective).
