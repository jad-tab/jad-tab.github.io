...
The second part will use slightly more theoretical tools. Please note that my point here is to present some useful tools to gain a high-level understanding of the data and some available options. The R code snippets below aren't useable without the original data. 
...
Of course these analyses are really far from being thorough and comprehensive: I've made some choices that severely restricted the data (by not considering the posts in Arabic, for instance). I also seemed to have based all the analysis on a single lexicon (which is itself limited and subjective). 
However, I dare to say that this approach could be a good starting point for a high-level understanding of the data.  
