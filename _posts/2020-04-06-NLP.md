...
Of course these analyses are really far from being thorough and comprehensive: we made some choices that severely restricted the data (by not considering the posts in Arabic). We then based all the analysis on a single lexicon (which is itself limited and subjective). 
However, this approach could be a good one for exploring the data we have at hand. 
