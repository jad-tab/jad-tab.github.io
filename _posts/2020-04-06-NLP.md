...
The second part will use a slightly more theoretical tool: Latent Dirichlet Allocation. Please note that my aim here is to present some useful tools to gain a high-level understanding of the data and some of the available options: the code snippets below aren't useable without the original data. 
...
We can clearly see two well-defined topics: the one in red is about connectivity issues and internet - the green one is more about billing concerns.  
So it seems that these are a thing in our customer's posts and messages.
